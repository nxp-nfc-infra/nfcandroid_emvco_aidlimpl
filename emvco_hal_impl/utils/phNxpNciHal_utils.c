...
int listInit(struct listHead *pList) {
  pList->pFirst = NULL;
  if (phOsalNfc_Thread_lock_mutex(&pList->mutex) != 0) {
...
int listDestroy(struct listHead *pList) {
  ...
  if (phOsalNfc_Thread_delete_mutex(&pList->mutex) != 0) {
...
int listAdd(struct listHead *pList, void *pData) {
  ...
  /* Create node */
  pNode = (struct listNode *)phOsalNfc_Memory_malloc(sizeof(struct listNode));
  if (pNode == NULL) {
    result = 0;
    NXPLOG_NCIHAL_E("Failed to phOsalNfc_Memory_malloc");
    goto clean_and_return;
  }
  pNode->pData = pData;
  pNode->pNext = NULL;
  phOsalNfc_create_mutex(&pList->mutex);
...
int listRemove(struct listHead *pList, void *pData) {
  ...
  phOsalNfc_Thread_lock_mutex(&pList->mutex);
  ...
  /* Deallocate the node */
  phOsalNfc_Memory_free(pRemovedNode);
...
phNxpNciHal_Monitor_t *phNxpNciHal_init_monitor(void) {
  ...
  if (nxpncihal_monitor == NULL) {
    nxpncihal_monitor = (phNxpNciHal_Monitor_t *)phOsalNfc_Memory_malloc(
  ...
  if (nxpncihal_monitor != NULL) {
    ...
    if (phOsalNfc_create_mutex(&nxpncihal_monitor->reentrance_mutex) != 0) {
      NXPLOG_NCIHAL_E("reentrance_mutex creation returned 0x%08x", errno);
      goto clean_and_return;
    }

    if (phOsalNfc_create_mutex(&nxpncihal_monitor->concurrency_mutex) != 0) {
      NXPLOG_NCIHAL_E("concurrency_mutex creation returned 0x%08x", errno);
      phOsalNfc_Thread_delete_mutex(&nxpncihal_monitor->reentrance_mutex);
      goto clean_and_return;
    }

    if (listInit(&nxpncihal_monitor->sem_list) != 1) {
      NXPLOG_NCIHAL_E("Semaphore List creation failed");
      phOsalNfc_Thread_delete_mutex(&nxpncihal_monitor->concurrency_mutex);
      phOsalNfc_Thread_delete_mutex(&nxpncihal_monitor->reentrance_mutex);
      goto clean_and_return;
    }
  } else {
...
clean_and_return:
  ...
  if (nxpncihal_monitor != NULL) {
    phOsalNfc_Memory_free(nxpncihal_monitor);
...
void phNxpNciHal_cleanup_cb_data(phNxpNciHal_Sem_t *pCallbackData) {
  /* Destroy semaphore */
  if (phOsalNfc_Thread_sem_destroy(&pCallbackData->sem)) {
...
void phNxpNciHal_releaseall_cb_data(void) {
  ...
  while (listGetAndRemoveNext(&phNxpNciHal_get_monitor()->sem_list,
                              (void **)&pCallbackData)) {
    pCallbackData->status = NFCSTATUS_FAILED;
    phOsalNfc_Thread_sem_post(&pCallbackData->sem);
  }
...
void phNxpNciHal_emergency_recovery(void) {
  if (nfcFL.chipType == pn548C2 && nfcdep_detected && discovery_cmd_len != 0) {
    ...
    if (phOsalNfc_Thread_create(
            &pthread, &attr, (void *(*)(void *))phNxpNciHal_core_reset_recovery,
            NULL) == 0) {
      return;
    }
  }
