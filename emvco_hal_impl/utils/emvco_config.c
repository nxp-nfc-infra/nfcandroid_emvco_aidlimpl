...
const NxpParam_t NXPConfig[] = {
    //////////////////////////////////////////////////////////////////////////////#
    {CFG_LOG_EXTNS_LOGLEVEL, TYPE_VAL, CONFIG_VAL 0x03},
    {CFG_LOG_NCIHAL_LOGLEVEL, TYPE_VAL, CONFIG_VAL 0x03},
    {CFG_LOG_NCIX_LOGLEVEL, TYPE_VAL, CONFIG_VAL 0x03},
    {CFG_LOG_NCIR_LOGLEVEL, TYPE_VAL, CONFIG_VAL 0x03},
    {CFG_LOG_FWDNLD_LOGLEVEL, TYPE_VAL, CONFIG_VAL 0x03},
    {CFG_LOG_TML_LOGLEVEL, TYPE_VAL, CONFIG_VAL 0x03},
    //////////////////////////////////////////////////////////////////////////////#
...
const NxpParam_t *NxpParamFind(const unsigned char key) {
  LOG_EXTNS_D("%s, NxpParamFind key=%c", __func__, key);
  int i;
  int listSize;
  ...
  if (listSize == 0) {
    LOG_EXTNS_D("%s, NxpParamFind returned NULL as listSize is zero", __func__);
    return NULL;
  }

  for (i = 0; i < listSize; ++i) {
    LOG_EXTNS_D("%s, NxpParamFind iterating key=%c", __func__,
                NXPConfig[i].key);
    LOG_EXTNS_D("%s,  NxpParamFind iterating type=%c", __func__,
                NXPConfig[i].type);
    if (NXPConfig[i].type == TYPE_DATA) {
      LOG_EXTNS_D("%s,  data len = %d\n", __func__,
    ...
    if (NXPConfig[i].key == key) {
      if (NXPConfig[i].type == TYPE_DATA) {
        LOG_EXTNS_D("%s found key %d, data len = %d\n", __func__, key,
                    *((unsigned char *)(NXPConfig[i].val)));
      } else {
        LOG_EXTNS_D("%s found key %d = (0x%lx)\n", __func__, key,
                    (long unsigned int)NXPConfig[i].val);
      }
      return &(NXPConfig[i]);
    }
...
**
*******************************************************************************/
int get_str_value(const char *key, char *pValue, unsigned long len) {
  if (!pValue)
    return FALSE;

  LOG_EXTNS_D("%s get_str_value key=%s", __func__, key);
  const NxpParam_t *pParam = NxpParamFind(*key);

  if (pParam == NULL) {
    LOG_EXTNS_D("%s get_str_value returned false pParam is NULL", __func__);
    return FALSE;
  }

  if ((pParam->type == TYPE_STR) && (pParam->val != NULL) &&
      (strlen(pParam->val) <= len)) {
    osal_memset(pValue, 0, len);
    osal_memcpy(pValue, pParam->val, strlen(pParam->val));
    LOG_EXTNS_D("%s get_byte_array_value *pValue=%s", __func__, pValue);
    LOG_EXTNS_D("%s get_str_value returned true at the end", __func__);
    return TRUE;
  }
  LOG_EXTNS_D("%s get_str_value returned false at the end", __func__);
  return FALSE;
}
...
**
*******************************************************************************/
int get_byte_array_value(const char *key, char *pValue, long bufflen,
                         long *readlen)
// int get_byte_array_value(unsigned char key, const void** pValue, long
// *readlen)
{
  (void)bufflen;
  LOG_EXTNS_D("%s, get_byte_array_value key=%s", __func__, key);
  LOG_EXTNS_D("%s, get_byte_array_value key=%c", __func__, *key);
  LOG_EXTNS_D("%s, get_byte_array_value key=%c", __func__, *pValue);
  if (!*pValue) {
    LOG_EXTNS_D("%s, get_byte_array_value pValue returned false", __func__);
    return FALSE;
  }
  const NxpParam_t *pParam = NxpParamFind(*key);

  if (pParam == NULL) {
    LOG_EXTNS_D("%s, get_byte_array_value pParam returned false", __func__);
    return FALSE;
  }
  ...
  if ((pParam->type == TYPE_DATA) && (pParam->val != 0)) {
    ...
    *readlen = (long)((unsigned char *)(pParam->val))[0];
    LOG_EXTNS_D("%s get_byte_array_value at the end returned true", __func__);
    return TRUE;
  }
  LOG_EXTNS_D("%s get_byte_array_value at the end returned false", __func__);
  return FALSE;
}
...
**
*******************************************************************************/
// int get_num_value(unsigned char key, void* pValue, unsigned long len)
int get_num_value(const char *key, void *pValue, unsigned long len) {
  if (!pValue)
    return FALSE;
