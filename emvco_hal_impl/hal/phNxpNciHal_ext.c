...
void *phNxpNciHal_process_emvco_mode_rsp_impl(void *vargp) {
  phTmlNfc_TransactInfo_t *pTransactionInfo = vargp;
  uint8_t *p_ntf = pTransactionInfo->pBuff;
  uint16_t p_len = pTransactionInfo->wLength;

  if (!issetEMVCoMode) {
    return NULL;
  }
  NXPLOG_NCIHAL_D(
      "phNxpNciHal_process_emvco_mode_rsp_impl data_len:%d issetEMVCoMode:%d",
      p_len, issetEMVCoMode);
  uint8_t msg_type, pbf, group_id, op_code, *p_data;

  p_data = p_ntf;
  NCI_MSG_PRS_HDR0(p_data, msg_type, pbf, group_id);
  NXPLOG_NCIHAL_D("phNxpNciHal_process_emvco_mode_rsp msg_type:%d group_id:%d ",
                  msg_type, group_id);
    ...
    case NCI_GID_RF_MANAGE:
      ...
      case MSG_RF_DISCOVER: {
        if (p_len == 4) {
          ...
          msg.eMsgType = EMVCO_POLLING_STARTED_MSG;
          msg.pMsgData = NULL;
          msg.Size = 0;
...
NFCSTATUS phNxpNciHal_start_emvco_mode(const int8_t emvco_config) {
  NXPLOG_NCIHAL_D("phNxpNciHal_start_emvco_mode send cmd_prop_act");
  phNxpNciHal_configure_pooling_tech(emvco_config);
  issetEMVCoMode = true;
  NFCSTATUS status = NFCSTATUS_SUCCESS;
  uint8_t cmd_prop_act[] = {0x2F, 0x02, 0x00};
  phNxpNciHal_write(sizeof(cmd_prop_act), cmd_prop_act);
  return status;
}
NFCSTATUS
...
NFCSTATUS phNxpNciHal_process_ext_rsp(uint8_t *p_ntf, uint16_t *p_len) {
  ...
  if (p_ntf[0] == 0x61 && p_ntf[1] == 0x05 && *p_len < 14) {
    ...
    if (*p_len <= 6) {
      phOsalNfc_Log_android_errorWriteLog(0x534e4554, "118152591");
    }
  ...
  if (p_ntf[0] == 0x61 && p_ntf[1] == 0x05 && p_ntf[2] == 0x15 &&
    ...
    if (nxpncihal_ctrl.nci_info.nci_version != NCI_VERSION_2_0) {
      if (*p_len <= (p_ntf[2] + 2)) {
        phOsalNfc_Log_android_errorWriteLog(0x534e4554, "181660091");
...
static NFCSTATUS phNxpNciHal_ext_process_nfc_init_rsp(uint8_t *p_ntf,
  ...
  if (p_ntf[0] == NCI_MT_RSP &&
    ...
    if (*p_len < 4) {
      phOsalNfc_Log_android_errorWriteLog(0x534e4554, "169258455");
      return NFCSTATUS_FAILED;
    }
    ...
    if (p_ntf[3] == CORE_RESET_TRIGGER_TYPE_CORE_RESET_CMD_RECEIVED ||
      ...
      if (len != *p_len - 1) {
        ...
        phOsalNfc_Log_android_errorWriteLog(0x534e4554, "121263487");
        return NFCSTATUS_FAILED;
      }
      ...
      phOsalNfc_Memory_set(print_buffer, 0, sizeof(print_buffer));
      for (i = 0; i < *p_len; i++) {
...
static NFCSTATUS phNxpNciHal_process_ext_cmd_rsp(uint16_t cmd_len,
  ...
  if (cmd_len < 3) {
    phOsalNfc_Log_android_errorWriteLog(0x534e4554, "153880630");
    status = NFCSTATUS_FAILED;
    goto clean_and_return;
  }
...
NFCSTATUS phNxpNciHal_write_ext(uint16_t *cmd_len, uint8_t *p_cmd_data,
  ...
  if (nfcFL.chipType == pn548C2 && p_cmd_data[0] == 0x20 &&
      p_cmd_data[1] == 0x02) {
    uint8_t temp;
