...
static void *phNxpNciHal_client_thread(void *arg) {
  ...
  while (p_nxpncihal_ctrl->thread_running == 1) {
    ...
    case PH_LIBNFC_DEFERREDCALL_MSG: {
      ...
      phNxpNciHal_process_emvco_mode_rsp(pTransactionInfo);
      REENTRANCE_UNLOCK();
    ...
    case EMVCO_POLLING_STARTED_MSG: {
      ...
      if (nxpncihal_ctrl.p_nfc_stack_cback != NULL) {
        /* Send the event */
        NXPLOG_NCIHAL_E("EMVCO_EVENT_ACTIVATED propagted to upper layer");
        (*nxpncihal_ctrl.p_nfc_stack_cback)(EMVCO_EVENT_ACTIVATED,
                                            HAL_NFC_STATUS_OK);
      }
      REENTRANCE_UNLOCK();
      break;
    ...
    case EMVCO_EVENT_START_MSG: {
      ...
      if (nxpncihal_ctrl.p_nfc_stack_cback != NULL) {
        /* Send the event */
        (*nxpncihal_ctrl.p_nfc_stack_cback)(EMVCO_EVENT_START_SUCCESS,
                                            HAL_NFC_STATUS_OK);
      }
      REENTRANCE_UNLOCK();
      break;
    ...
    case EMVCO_EVENT_STOP_MSG: {
      ...
      if (nxpncihal_ctrl.p_nfc_stack_cback != NULL) {
        /* Send the event */
        (*nxpncihal_ctrl.p_nfc_stack_cback)(EMVCO_EVENT_STOP_IN_PROGRESS,
                                            HAL_NFC_STATUS_OK);
      }
      REENTRANCE_UNLOCK();
      break;
...
void phNxpNciHal_handleNfcStateChanged(int32_t nfc_state) {
  NXPLOG_NCIHAL_D("%s nfc_state:%d", __func__, nfc_state);
  nfcStatus = nfc_state;
  if (nfcStatus == STATE_OFF) {
...
int phNxpNciHal_openImpl(nfc_stack_callback_t *p_cback,
  ...
  if (nfcStatus != STATE_OFF) {
    if (0 != phOsalNfc_Thread_sem_init(&nfcStatusSemaphore, 0, 1)) {
  ...
  if (in_isStartEMVCo) {
    if (isValidPollingTechnology(emvco_config)) {
      ...
      if (hal_open_status == NFCSTATUS_SUCCESS) {
        ...
        phNxpNciHal_start_emvco_mode(emvco_config);
      } else {
        phLibNfc_Message_t msg;
        msg.eMsgType = EMVCO_EVENT_START_FAILED_MSG;
        msg.pMsgData = NULL;
        msg.Size = 0;
        phTmlNfc_DeferredCall(gpphTmlNfc_Context->dwCallbackThreadId, &msg);
      }
    ...
    if (nxpncihal_ctrl.halStatus == HAL_STATUS_OPEN) {
      NXPLOG_NCIHAL_D("%s HAL is open, phNxpNciHal_stop_emvco_mode", __func__);
      phNxpNciHal_stop_emvco_mode();
    } else {
      NXPLOG_NCIHAL_D("%s HAL is not open, No need to clean up", __func__);
    }
  }
...
void phNxpNciHal_doSetEMVCoMode(const int8_t emvco_config,
                                bool_t in_isStartEMVCo) {
  NXPLOG_NCIHAL_D("%s emvco_config:%d in_isStartEMVCo:%d", __func__,
...
int phNxpNciHal_MinOpen() {
  ...
  CONCURRENCY_LOCK();
  phOsalNfc_Memory_set(&nxpncihal_ctrl, 0x00, sizeof(nxpncihal_ctrl));
  phOsalNfc_Memory_set(&tOsalConfig, 0x00, sizeof(tOsalConfig));
  phOsalNfc_Memory_set(&tTmlConfig, 0x00, sizeof(tTmlConfig));
  phOsalNfc_Memory_set(&nxpprofile_ctrl, 0, sizeof(phNxpNciProfile_Control_t));

  /*Init binary semaphore for Spi Nfc synchronization*/
  if (0 != phOsalNfc_Thread_sem_init(&nxpncihal_ctrl.syncSpiNfc, 0, 1)) {
    NXPLOG_NCIHAL_E("phOsalNfc_Thread_sem_init() FAiled, errno = 0x%02X",
                    errno);
    goto clean_and_return;
  }
  ...
  if (nfc_dev_node == NULL) {
    ...
    goto clean_and_return;
  } else if (!GetNxpStrValue(NAME_CFG_NXP_NFC_DEV_NODE, nfc_dev_node,
                             max_len)) {
    NXPLOG_NCIHAL_D(
  ...
  if (nfcFL.chipType == pn548C2) {
    phOsalNfc_Memory_set(discovery_cmd, 0, sizeof(discovery_cmd));
    discovery_cmd_len = 0;
  }
  ...
  if (wConfigStatus != NFCSTATUS_SUCCESS) {
    ...
    if (nfc_dev_node != NULL) {
      phOsalNfc_Memory_free(nfc_dev_node);
      nfc_dev_node = NULL;
    }
  }

  /* Create the client thread */
  ret_val = phOsalNfc_Thread_create(&nxpncihal_ctrl.client_thread, NULL,
                                    phNxpNciHal_client_thread, &nxpncihal_ctrl);
  if (ret_val != 0) {
    NXPLOG_NCIHAL_E("phOsalNfc_Thread_create failed");
...
void phNxpNciHal_discovery_cmd_ext(uint8_t *p_cmd_data, uint16_t cmd_len) {
  ...
  if (cmd_len > 0 && cmd_len <= sizeof(discovery_cmd)) {
    phOsalNfc_Memory_copy(discovery_cmd, p_cmd_data, cmd_len);
...
int phNxpNciHal_write_internal(uint16_t data_len, const uint8_t *p_data) {
  ...
  if (data_len > NCI_MAX_DATA_LEN) {
    ...
    phOsalNfc_Log_android_errorWriteLog(0x534e4554, "121267042");
    goto clean_and_return;
  }
  /* Create local copy of cmd_data */
  phOsalNfc_Memory_copy(nxpncihal_ctrl.p_cmd_data, p_data, data_len);
...
retry:
  ...
  if (cb_data.status != NFCSTATUS_SUCCESS) {
    ...
    if (nxpncihal_ctrl.retry_cnt++ < MAX_RETRY_COUNT) {
      ...
      phOsalNfc_Thread_sem_post(&(nxpncihal_ctrl.syncSpiNfc));
      ...
      if (nxpncihal_ctrl.p_nfc_stack_data_cback != NULL &&
        ...
        phOsalNfc_Memory_copy(nxpncihal_ctrl.p_rx_data, reset_ntf,
                              sizeof(reset_ntf));
...
static void phNxpNciHal_read_complete(void *pContext,
  ...
  if (pInfo->wStatus == NFCSTATUS_SUCCESS) {
    ...
    phOsalNfc_Thread_sem_getvalue(&(nxpncihal_ctrl.syncSpiNfc), &sem_val);
...
int phNxpNciHal_core_initialized(uint16_t core_init_rsp_params_len,
  ...
  isfound = GetNxpByteArrayValue(NAME_NXP_CORE_CONF, (char *)buffer, bufflen,
                                 &retlen);
  NXPLOG_NCIHAL_D("NAME_NXP_CORE_CONF Settings Found - %d Len: %ld", isfound,
                  retlen);
  if (retlen > 0) {
    /* NXP ACT Proprietary Ext */
    status = phNxpNciHal_send_ext_cmd(retlen, buffer);
...
int phNxpNciHal_close(bool bShutdown) {
  ...
  phOsalNfc_Thread_sem_destroy(&nxpncihal_ctrl.syncSpiNfc);
  ...
  if (NULL != gpphTmlNfc_Context->pDevHandle) {
    ...
    if (0 !=
        phOsalNfc_Thread_join(nxpncihal_ctrl.client_thread, (void **)NULL)) {
...
int phNxpNciHal_check_ncicmd_write_window(uint16_t cmd_len, uint8_t *p_cmd) {
  ...
  if ((p_cmd[0] & 0xF0) == 0x20) {
    ...
    while ((s = phOsalNfc_Thread_sem_timedwait_monotonic_np(
                &nxpncihal_ctrl.syncSpiNfc, &ts)) == -1 &&
           errno == EINTR)
...
retry_send_ext:
  ...
  if (isfound > 0) {
    ...
    if (send_flag == true) {
      ...
      phOsalNfc_Memory_copy(&set_rf_cmd[4], &phNxpNciRfSet.p_rx_data[5], 7);
...
void phNxpNciHal_configFeatureList(uint8_t *msg, uint16_t msg_len) {
  tNFC_chipType chipType = getChipType(msg, msg_len);
...
static void phNxpNciHal_print_res_status(uint8_t *p_rx_data, uint16_t *p_len) {
  ...
  if (p_rx_data[0] == 0x40 && (p_rx_data[1] == 0x02 || p_rx_data[1] == 0x03)) {
    ...
    if (phNxpNciClock.isClockSet) {
      ...
      if (*p_len > len) {
        phOsalNfc_Log_android_errorWriteLog(0x534e4554, "169257710");
      } else {
        len = *p_len;
      }
      for (i = 0; i < len; i++) {
      ...
      if (*p_len > len) {
        phOsalNfc_Log_android_errorWriteLog(0x534e4554, "169258733");
      } else {
        len = *p_len;
      }
      for (i = 0; i < len; i++) {
      ...
      if (*p_len > len) {
        phOsalNfc_Log_android_errorWriteLog(0x534e4554, "169258884");
      } else {
        len = *p_len;
      }
      for (i = 8; i < len; i++) {
