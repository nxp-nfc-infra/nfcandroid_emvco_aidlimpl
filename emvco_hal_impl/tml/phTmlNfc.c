  ...
  /* Validate Input parameters */
    ...
    if (NULL == gpphTmlNfc_Context) {
      ...
      if (NFCSTATUS_SUCCESS != wInitStatus) {
        ...
        if (phOsalNfc_create_mutex(&gpphTmlNfc_Context->readInfoUpdateMutex) !=
            0) {
...
static NFCSTATUS phTmlNfc_StartThread(void) {
  ...
  pthread_create_status =
      phOsalNfc_Thread_create(&gpphTmlNfc_Context->readerThread, NULL,
    ...
    if (1 == gpphTmlNfc_Context->tReadInfo.bEnable) {
      ...
      if (NULL != gpphTmlNfc_Context->pDevHandle) {
        ...
        if (-1 == dwNoBytesWrRd) {
          ...
          phOsalNfc_Thread_lock_mutex(&gpphTmlNfc_Context->readInfoUpdateMutex);
          ...
          phOsalNfc_Thread_unlock_mutex(
              &gpphTmlNfc_Context->readInfoUpdateMutex);
...
void phTmlNfc_CleanUp(void) {
  ...
  if (NULL != gpphTmlNfc_Context->pDevHandle) {
    (void)phTmlNfc_i2c_nfcc_reset(gpphTmlNfc_Context->pDevHandle,
                                  MODE_POWER_OFF);
...
NFCSTATUS phTmlNfc_Shutdown(void) {
  ...
  if (NULL != gpphTmlNfc_Context) {
    ...
    sem_post(&gpphTmlNfc_Context->postMsgSemaphore);
    usleep(1000);
    phOsalNfc_Thread_delete_mutex(&gpphTmlNfc_Context->readInfoUpdateMutex);
    if (0 != phOsalNfc_Thread_join(gpphTmlNfc_Context->readerThread,
                                   (void **)NULL)) {
      NXPLOG_TML_E("Fail to kill reader thread!");
    }
    if (0 != phOsalNfc_Thread_join(gpphTmlNfc_Context->writerThread,
                                   (void **)NULL)) {
  ...
  if (NULL != gpphTmlNfc_Context) {
    if ((gpphTmlNfc_Context->pDevHandle != NULL) && (NULL != pBuffer) &&
      ...
      if (!gpphTmlNfc_Context->tReadInfo.bThreadBusy) {
        ...
        phOsalNfc_Thread_unlock_mutex(&gpphTmlNfc_Context->readInfoUpdateMutex);
        sem_post(&gpphTmlNfc_Context->rxSemaphore);
      } else {
...
NFCSTATUS phTmlNfc_IoCtl(phTmlNfc_ControlCode_t eControlCode) {
  ...
  if (NULL == gpphTmlNfc_Context) {
    ...
    case phTmlNfc_e_ResetDevice: {
      /*Reset PN54X*/
      phTmlNfc_i2c_nfcc_reset(gpphTmlNfc_Context->pDevHandle, MODE_POWER_ON);
      usleep(100 * 1000);
      phTmlNfc_i2c_nfcc_reset(gpphTmlNfc_Context->pDevHandle, MODE_POWER_OFF);
      usleep(100 * 1000);
      phTmlNfc_i2c_nfcc_reset(gpphTmlNfc_Context->pDevHandle, MODE_POWER_ON);
      break;
    }
